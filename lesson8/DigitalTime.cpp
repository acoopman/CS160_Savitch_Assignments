...
DigitalTime::DigitalTime(int the_hour, int the_minute)
{
	if (the_hour < 0 || the_hour > 23 || the_minute < 0 || the_minute > 59)
	{
	  ...
	  //exit(1);
	  throw DigitalTimeException(1,"Illegal argument to DigitalTime constructor.");
	}
	else
	{
		hour = the_hour;
...
void read_minute(istream& ins, int& the_minute)
{
	...
	if (!(isdigit(c1) && isdigit(c2)))
	{
	  ...
	  //exit(1);
	  throw DigitalTimeException(2,"Error illegal input to read_minute\n");
	}
	...
	if (the_minute < 0 || the_minute > 59)
	{
	  //cout << "Error illegal input to read_minute\n";
	  //exit(1);
	  throw DigitalTimeException(3,"Error illegal input to read_minute\n");
	}
}
...
void read_hour(istream& ins, int& the_hour)
{
	...
	if ( !( isdigit(c1) && (isdigit(c2) || c2 == ':' ) ) )
	{
	  ...
	  //exit(1);
	  throw DigitalTimeException(4,"Error illegal input to read_hour\n");
	}
	...
	else //(isdigit(c1) && isdigit(c2))
	{
		...
		if (c2 != ':')
		{
		  //cout << "Error illegal input to read_hour\n";
		  //exit(1);
		  throw DigitalTimeException(5,"Error illegal input to read_hour\n");
		}
	}
	...
	if ( the_hour < 0 || the_hour > 23 )
	{
	  //cout << "Error illegal input to read_hour\n";
        //exit(1);
	  throw DigitalTimeException(6,"Error illegal input to read_hour\n");
	}
}
